...
export default function DynamicInputList({
  ...
  return (
    <div>
      <label className="block text-sm font-medium text-[var(--primary)] mb-2">{label}</label>
        ...
        <div key={index} className="flex items-start gap-2 mb-2">
          ...
          {isInstruction ? (
            <textarea
              ...
              rows={2}
              className="flex-1 bg-white px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[var(--accent)] placeholder-gray-400"
              maxLength={1000}
            />
          ) : (
            <input
          ...
          <div className="flex gap-2">
            ...
            {index === items.length - 1 && (
              <button
                ...
                className="px-3 py-2 bg-[var(--primary)] text-white rounded-md hover:bg-[var(--hover)]"
              >
                +
              </button>
            )}
